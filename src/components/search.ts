...
export function strSearch(val:string,root:Element,result:SearchResult[],isCaseSensitive:boolean){
    if(!result) return
...
export function regexpSearch(regex:RegExp,root:Element,result:SearchResult[]){
    if(!result) return
 ...
 * @returns 
 */
export function search(val:string,root:Element,options?:{isCaseSensitive:boolean,isWholeWord:boolean,isRegexp:boolean}){
    if(!root.childNodes || !val || val == "*") return 

    let result:SearchResult[] = []
    options = options?options:{isCaseSensitive:false,isWholeWord:false,isRegexp:false}
...
export function highlightSearchResult(searchResult:SearchResult[],highlightClass:string){
...
export function selectMatch(result:SearchResult[]|HighlightResult[],index:number,options?:{selectedClass:string}){
    if(!result || index<0 || index>=result.length) return

    let data = result[index]
    let {selectedClass} = options?options:{selectedClass:"selected-highlight"}
